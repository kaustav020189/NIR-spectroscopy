...
def get_split_data(files: list, split_params: dict, test_size: float) -> tuple:
    ...
    if type(files) == list:                                      # multiple files
        ...
        X = big_df.drop(axis=1,columns=split_params['drop_columns']) # drop the last column, so we have exactly 256 features
...
def pipeline(pipeline_params:dict):
    ...
    if model == 'pls':
        ...
        if pre_processor == 'savgol0' or pre_processor == 'savgol1' or pre_processor == 'savgol2':
            window_range = pipeline_params['preprocessor-params']['savgol']['window_size_range']
            polyorder_range = pipeline_params['preprocessor-params']['savgol']['polyorder_range']
            derivative = pipeline_params['preprocessor-params']['savgol']['derivative']
            ...
            if showPlots:
                plot(x_axis_data=wl, y_axis_data=X_train_preprocessed, title='Post SavGol plot (Deriv : '+str(derivative)+' )', xlabel='wavelength', ylabel='absorbance')
        elif pre_processor == 'snv':
            X_train_preprocessed, X_test_preprocessed = snv(data)
            if showPlots:
                plot(x_axis_data=wl, y_axis_data=X_train_preprocessed, title='Post SNV Plot', xlabel='wavelength', ylabel='absorbance')
        elif pre_processor == 'msc':
            X_train_preprocessed, X_test_preprocessed = msc(data)
            if showPlots:
                plot(x_axis_data=wl, y_axis_data=X_train_preprocessed, title='Post MSC plot', xlabel='wavelength', ylabel='absorbance')
        ...
        best_n_comp_val = optimize_and_validate_pls(X_train_preprocessed, y_train, validation_params, showPlot=showPlots)
        # SAVE OPTIMISED HYPERPARAMETER VALUE
        logger.info("Optimized n_comp for PLS {}".format(best_n_comp_val))
        #print("Optimized hyperparameter after model validation - n_comp for PLS : "+str(best_n_comp_val))

        # [ STEP 3 ] Model evaluation
        logger.info("[MODEL EVALUATION]")
        error = evaluate_pls(X_train_preprocessed, X_test_preprocessed, y_train, y_test, best_n_comp_val, showPlot=True)
...
def savgol(data, window_size_range, polyorder_range, deriv=0, showPlot=False):
    ...
    if showPlot:
        ...
        heatmap_plot(mse_scores, range_min, range_max, polyorder_range, window_size_range, 'MSE scores after SAVGOL  (Deriv : '+str(deriv)+' )', 'Polyorder', 'Window size')
    ...
    best_window_size, best_polyorder = window_size_range[min_mse_val_coord[0][0]], polyorder_range[min_mse_val_coord[0][1]]
...
def optimize_and_validate_pls(X, y, validation_params, showPlot=False):
    ...
    for i in component:
        ...
        # Check if KFolds of StratifiedKFolds
...
def evaluate_pls(X_train, X_test, y_train, y_test, min_pls_n_comp, showPlot=False):
